    ...
    enableAbsoluteBottomNavForMobile() {
        ...
        try {
            ...
            const updatePosition = () => {
                ...
                const newTop = Math.max(0, scrollY + viewportHeight - navHeight - bottomOffset);
                
                // Apply position only if changed (avoids layout thrash)
                if (this._bottomNavY !== newTop) {
                    bottomNav.style.position = 'absolute';
                    bottomNav.style.bottom = 'auto';
                    bottomNav.style.top = newTop + 'px';
                    bottomNav.style.left = '50%';
                    bottomNav.style.transform = 'translateX(-50%)';
                    bottomNav.style.zIndex = '9999';
                    this._bottomNavY = newTop;
                }
