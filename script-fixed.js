...
class ScheduleApp {
    ...
    setupEventListeners() {
        ...
        // Navigation buttons (только правая кнопка)
        const nextWeek = document.getElementById('nextWeek');
        if (nextWeek) {
            nextWeek.addEventListener('click', () => this.navigateWeek(1));
            // Добавляем длительное нажатие для перехода назад
            let longPressTimer;
            nextWeek.addEventListener('touchstart', (e) => {
                longPressTimer = setTimeout(() => {
                    this.navigateWeek(-1);
                    this.vibrate();
                }, 800);
            });
            nextWeek.addEventListener('touchend', () => {
